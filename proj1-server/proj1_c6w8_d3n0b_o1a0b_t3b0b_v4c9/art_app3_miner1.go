...
func main() {
	...
	if inkRemaining > 480 {
		// add a square
		shapeHash4, blockHash4, ink4, err := canvas.AddShape(validateNum, blockartlib.PATH, "M 0 0 H 20 V 20 h -20 Z", "green", "black")
		fmt.Printf("%v %v %v %v \n", shapeHash4, blockHash4, ink4, err)
		if checkError(err) != nil {
			fmt.Printf("FAIL SQUARE %v %v %v %v \n", shapeHash4, blockHash4, ink4, err)
			fmt.Println(err)
		}
	}
